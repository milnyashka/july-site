-- Статусы Bronze/Silver/Gold: только покупки во вкладке «Продукты» (таблица purchases).
-- Маркет (marketplace_purchases) НЕ влияет на уровень и НЕ получает tier-скидку.

create or replace function public.get_product_spend_usd(p_user_id uuid)
returns numeric
language sql
stable
...
begin
  ...
  -- Только purchases (вкладка Продукты), НЕ маркет
